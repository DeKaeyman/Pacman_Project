...
TEST_CASE("Stopwatch basic sequence: reset -> tick -> dt/elapsed sane",
          "[stopwatch]") {
...
TEST_CASE("Stopwatch measures time between ticks (~sleep 10ms)",
          "[stopwatch]") {
...
TEST_CASE(
    "Stopwatch monotonic elapsed, delta near-zero for quick successive tick",
    "[stopwatch]") {
  auto &sw = Stopwatch::getInstance();
  sw.reset();
  sw.tick();
