...
namespace {
    ...
    struct DetachingObserver : Observer { // Observer that detaches itself on first event
        Subject* subject{};
        int received{0};
        void onEvent(const Event& e) override {
            (void)e;
            ++received;
            if (subject) subject->detach(this); // detach during notify
        }
    };
}
...
TEST_CASE("Detach during notify does not break iteration (copy-notify)", "[observer]") {
    TestSubject s;
