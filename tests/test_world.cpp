...
TEST_CASE("World add/remove assigns incremental ids and handles empties",
          "[world]") {
  MockFactory mf;
  World w(mf);
...
TEST_CASE(
    "World update calls Entity::update and records solid-solid collisions",
    "[world]") {
  MockFactory mf;
  World w(mf);

  auto idA =
      w.addEntity(std::make_unique<TestEntity>(Rect{0, 0, 10, 10}, true));
  auto idB =
      w.addEntity(std::make_unique<TestEntity>(Rect{5, 5, 10, 10}, true));
  w.addEntity(std::make_unique<TestEntity>(Rect{0, 0, 100, 100},
                                           false)); // Non solid ignored

  w.update(0.016); // Simulate frame update

  int updatedCount = 0;
  w.forEachEntity([&](Entity &e) {
    (void)e;
    ++updatedCount;
  });
  ...
  REQUIRE(((p.first == idA && p.second == idB) ||
           (p.first == idB && p.second == idA)));
}
