...
namespace pacman::logic {
    ...
    Random::Random() : engine_(std::mt19937{5489u}) {} // Initialize RNG with fixed seed
    ...
    void Random::seedFromDevice() {
        ...
        std::seed_seq seq{ rd(), rd(), rd(), rd(), rd(), rd(), rd(), rd() }; // Build complex seed sequence
    ...
    double Random::uniformReal(double a, double b) {
        std::uniform_real_distribution<double> dist(a, b); // Real valued uniform distribution
    ...
    std::size_t Random::choiceIndex(std::size_t count) {
        if (count == 0) throw std::out_of_range("choiceIndex on empty range"); // Prevents invalid range
        std::uniform_int_distribution<std::size_t> dist(0, count - 1); // Uniform over valid
        return dist(engine_); // Return chosen index
    }

    void Random::shuffleIndices(std::vector<std::size_t> &indices) {
        std::shuffle(indices.begin(), indices.end(), engine_); // Randomly reorder elements using current engine
    }
}