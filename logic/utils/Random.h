...
public:
  ...
  double
  uniformReal(double a,
              double b);    // Returns random floating point uniformly in [a, b]
  bool bernoulli(double p); // Returns true with probability p
  double normal(double mean,
                double stddev); // Returns value from normal distribution N

  std::size_t
  choiceIndex(std::size_t count); // Returns random index in [0, count-1]
  void shuffleIndices(std::vector<std::size_t>
                          &indices); // Shuffles a vector of indices in place
...
private:
  ...
  std::mt19937
      engine_; // Mersenne Twister PRNG engine providing deterministic sequences
};
} // namespace pacman::logic