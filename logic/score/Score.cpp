...
void Score::onEvent(const pacman::logic::Event &e) {
  ...
  case EventType::Collected: { // A collectable (coin/fruit) has been picked up
    if (auto payload = std::get_if<CollectedPayload>(&e.payload)) {
      ...
      int bonusScore =
          static_cast<int>(base * multiplier); // Apply chain multiplier
...
void Score::saveHighscores(const std::string &path,
...
std::vector<int> Score::updateHighscores(const std::vector<int> &current,
                                         int newScore) {
