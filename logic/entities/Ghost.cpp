...
Ghost::Ghost(const pacman::logic::Rect &startBounds,
             pacman::logic::GhostKind kind, double speed)
    : bounds_(startBounds), spawnBounds_(startBounds),
      direction_(Direction::None), speed_(speed), kind_(kind) {
...
void Ghost::update(double dt) {
  ...
  float dist =
      static_cast<float>(speed_ * dt); // Distance ghost moves this frame
