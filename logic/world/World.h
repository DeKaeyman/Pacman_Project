...
inline bool
intersects(const Rect &a, const Rect &b,
...
public:
  ...
  void
  update(double dt); // Ticks all entities and calculates possible collisions

  template <typename F> void forEachEntity(F &&f) {
...
private:
  ...
  std::vector<std::pair<EntityId, EntityId>>
      lastCollisions_; // Collisions of last frame
  std::vector<std::pair<EntityId, EntityId>>
      lastOverlaps_; // Overlaps of last frame (non-blocking)
