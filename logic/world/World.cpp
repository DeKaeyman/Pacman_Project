...
bool World::removeEntity(EntityId id) {
  ...
  const bool removed =
      it != entities_.end();            // removed = something has been removed
...
const Entity *
World::get(EntityId id) const { // Const variant for only read access
  for (auto &p : entities_)
    if (p && p->id() == id)
      return p.get(); // Search for a valid entity based on the id
  return nullptr;
...
void World::handlePacManTurning(double dt) {
  for (auto &e : entities_) {
    ...
    checkPacmanDesiredDirection(
        *pac, dt); // Try to turn Pacman if buffered direction is valid
  }
}
...
void World::updateCollisions() {
  lastCollisions_
      .clear(); // Calculate all pairs of entities that overlap after update
  ...
  for (std::size_t i = 0; i < n; i++) {
    ...
    for (std::size_t j = i + 1; j < n;
         j++) { // i+1 prevent double checks (a,a)=mù:
...
void World::loadLevel(const pacman::logic::TileMap &map) {
  ...
  for (int y = 0; y < TileMap::Height; y++) {  // Loop over all rows
    for (int x = 0; x < TileMap::Width; x++) { // Loop over all columns
      ...
      const Rect r =
          tileMap_.tileRect(x, y); // World space rectangle for that tile
