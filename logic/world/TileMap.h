...
namespace pacman::logic {
    ...
    enum class TileType : std::uint8_t { // Defines all possible tile contents in the logical grid
    ...
    public:
        ...
        TileType at(int x, int y) const noexcept { // Get the tile type at (x,y)
            if (!inBounds(x, y)) return TileType::Empty; // Out of bounds
    ...
    inline TileMap::TileMap() : tiles_(Width * Height, TileType::Empty) { // ASCII representation of the map layout
