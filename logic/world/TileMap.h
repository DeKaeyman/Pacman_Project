...
public:
  ...
  bool inBounds(int x, int y)
      const noexcept { // Check if the tile coordinate lies within map
  ...
  Rect tileRect(int x, int y)
      const noexcept { // Convert tile coords to world space rectangle
    Rect r{};
...
inline TileMap::TileMap()
  ...
  for (int y = 0; y < Height; y++) {  // Iterate trough each row
    for (int x = 0; x < Width; x++) { // Iterate trough each column
      ...
      tiles_[y * Width + x] =
          t; // Store tile in 1D array at its flattened index
    }
  }
}
} // namespace pacman::logic