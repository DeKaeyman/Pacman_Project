...
std::pair<float, float> Camera::worldToPixel(float wx,
                                             float wy) const noexcept {
  // Uniform scale: same for X and Y so world units stay square
  const float scale =
      static_cast<float>(std::min(width_, height_)) / WorldExtent;
...
PixelRect Camera::worldToPixel(const pacman::logic::Rect &worldRect) const
    noexcept { // Converts a world space rectangle into a pixel space rectangle
  auto [px, py] =
      worldToPixel(worldRect.x, worldRect.y); // Convert the top left corner
