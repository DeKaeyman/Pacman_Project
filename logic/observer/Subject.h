...
namespace pacman::logic {
    ...
    public:
        void attach(Observer* o) { // Register an observer if not already present
            if (!o) return;
            if (std::find(observers_.begin(), observers_.end(), o) == observers_.end()) observers_.push_back(o); // Avoid duplicates
        }
    ...
    protected:
        void notify(const Event& e) { // Broadcast an event to all observers
            ...
            for (auto* o : copy) if (o) o->onEvent(e); // Call each observer if still valid
        }
