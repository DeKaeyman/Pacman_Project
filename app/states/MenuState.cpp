...
MenuState::MenuState(pacman::app::StateManager &m)
    : State(m) { // Constructor, forwards StateManager to base class
  font_.loadFromFile(
      "assets/fonts/arial.ttf"); // Load a font for displaying text
  highscores_ = pacman::logic::Score::loadHighscores(
      highscorePath_); // Load top 5 highscores from logic layer
}

void MenuState::handleEvent(const sf::Event &e) {
  if (e.type == sf::Event::KeyPressed &&
      e.key.code ==
          sf::Keyboard::Enter) { // When enter is pressed -> Go to level state
  ...
  if (e.type == sf::Event::MouseButtonPressed &&
      e.mouseButton.button ==
          sf::Mouse::Left) { // When left mouse button is pressed -> check if
...
void MenuState::draw(sf::RenderWindow &w) {
  ...
  for (int i = 0; i < 5; i++) { // Build five score lines
    int value = (i < static_cast<int>(highscores_.size()))
                    ? highscores_[i]
                    : 0; // Use real score if available

    std::ostringstream oss;
    oss << (i + 1) << ". " << std::setw(5) << std::setfill('0')
        << value; // Stream for zero padded formatting
  ...
  float centerY =
      static_cast<float>(windowHeight_) * 0.75f; // 75% down the screen

  sf::RectangleShape button(
      {BUTTON_WIDTH, BUTTON_HEIGHT}); // Create button shape with fixed size
