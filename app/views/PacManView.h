...
namespace pacman::app {
    ...
    public:
        explicit PacManView(const std::shared_ptr<pacman::logic::PacMan>& model); // Store model and init sprite

        void draw(sf::RenderWindow& window) override; // Draw Pacman with correct frame and position
        void onEvent(const pacman::logic::Event& e) override; // React to direction/state changes from model
    ...
    private:
        ...
        void updateSpriteFrame(); // Picks correct animation frame based on direction + time
    };
}