...
public:
  explicit FruitView(const std::shared_ptr<pacman::logic::Fruit>
                         &model); // Uses model to initialize

  void draw(sf::RenderWindow &window) override; // Draw fruit sprite to window
  void
  onEvent(const pacman::logic::Event &e) override; // React to fruit collection

private:
  std::shared_ptr<pacman::logic::Fruit>
