...
namespace pacman::app {
    ...
    public:
        explicit GhostView(const std::shared_ptr<pacman::logic::Ghost>& model); // Store model + load texture

        void draw(sf::RenderWindow& window) override; // Draw ghost sprite on screen
        void onEvent(const pacman::logic::Event& e) override; // React to movement/direction/fear event

    private:
        std::shared_ptr<pacman::logic::Ghost> model_; // Ghost model this view follows
        sf::Sprite sprite_; // Visual sprite for the ghost

        pacman::logic::Direction direction_{pacman::logic::Direction::None}; // Current facing direction
        ...
        void updateSpriteFrame(); // Select correct frame based on direction + time + fear mode
    };
}