...
sf::IntRect spriteRectFromGrid(const sf::Texture &tex, unsigned int col,
  ...
  const float cellW = static_cast<float>(size.x) /
                      static_cast<float>(SHEET_COLS); // Width of one cell
  const float cellH = static_cast<float>(size.y) /
                      static_cast<float>(SHEET_ROWS); // Height of one cell
...
void PacManView::onEvent(const pacman::logic::Event &e) {
  ...
  if (e.type ==
      EventType::StateChanged) { // StateChanged encodes direction changes
    if (const auto *payload =
            std::get_if<pacman::logic::StateChangedPayload>(&e.payload)) {
...
void PacManView::updateSpriteFrame() {
  ...
  auto &sw =
      pacman::logic::Stopwatch::getInstance(); // Global stopwatch singleton
  const double t = sw.elapsed(); // Total elapsed time since start/reset

  const unsigned int row =
      pickRowFor(direction_, t); // Select proper animation row
  const auto rect =
      spriteRectFromGrid(texture_, PACMAN_COL, row); // Compute UV rectangle
...
void PacManView::draw(sf::RenderWindow &window) {
  ...
  pacman::logic::Rect r =
      model_->bounds(); // World space bounding box of Pacman
  auto pixelRect = camera_->worldToPixel(r); // Convert to pixel space rectangle

  const auto &texRect =
      sprite_.getTextureRect(); // Current sprite frame size in pixels

  float scale = static_cast<float>(pixelRect.w) /
                static_cast<float>(texRect.width); // Uniform scaling
