...
constexpr const char *SPRITE_SHEET_PATH =
    "assets/sprites/sprite.png"; // Path to sprite sheet asset
...
sf::IntRect spriteRectFromGrid(const sf::Texture &tex, unsigned int col,
  ...
  const float cellW = static_cast<float>(size.x) /
                      static_cast<float>(SHEET_COLS); // Width of one grid cell
  const float cellH = static_cast<float>(size.y) /
                      static_cast<float>(SHEET_ROWS); // Height of one grid cell
...
void CoinView::ensureTextureLoaded() {
  ...
  if (!texture_.loadFromFile(SPRITE_SHEET_PATH))
    return;              // Attempt to load sprite sheet
  textureLoaded_ = true; // Mark texture as loaded
}

CoinView::CoinView(const std::shared_ptr<pacman::logic::Coin> &model)
    : model_(model) {
  ...
  if (textureLoaded_) {           // Setup sprite only if texture is loaded
    ...
    sprite_.setTextureRect(spriteRectFromGrid(
        texture_, COIN_COL, COIN_ROW)); // Select only the coin's region
  }
}

void CoinView::onEvent(const pacman::logic::Event &e) {
  if (e.type ==
      pacman::logic::EventType::Collected) { // Coin has been collected
...
void CoinView::draw(sf::RenderWindow &window) {
  if (!visible_)
    return; // Skip drawing if already collected
  if (!model_)
    return; // No model -> nothing to draw
  if (!model_->active)
    return; // Inactive coin -> don't display
  if (!camera_)
    return; // Camera required for world -> pixel mapping
  if (!textureLoaded_)
    return; // No texture loaded -> skip drawing
  ...
  const auto &texRect =
      sprite_.getTextureRect(); // Current sprite size in texture

  float scale =
      static_cast<float>(pixelRect.w) /
      static_cast<float>(texRect.width); // Uniform scale so coin fits tile
