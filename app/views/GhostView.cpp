...
namespace pacman::app {
    ...
    namespace {
        ...
        unsigned int columnForKind(pacman::logic::GhostKind kind) {
            ...
            switch (kind) {
                case GhostKind::A: return 13;
                case GhostKind::B: return 14;
                case GhostKind::C: return 15;
                case GhostKind::D: return 16;
            }
            return 13;
        }
    ...
    void GhostView::ensureTextureLoaded() {
        ...
        if (!texture_.loadFromFile(SPRITE_SHEET_PATH)) return; // Fail silently if missing
    ...
    void GhostView::onEvent(const pacman::logic::Event& e) {
        ...
        if (e.type == EventType::StateChanged) { // Only listen for direction/fear changes
            if (const auto* payload = std::get_if<StateChangedPayload>(&e.payload)) {
                switch (payload->code) { // Model encodes direction or fear trigger as integer codes
    ...
    void GhostView::updateSpriteFrame() {
        if (!textureLoaded_) return; // No sprite sheet loaded
        if (!model_) return; // Safety check

        auto& sw = pacman::logic::Stopwatch::getInstance(); // Global stopwatch for animation time
    ...
    void GhostView::draw(sf::RenderWindow& window) {
        ...
        if (!model_->active) return; // Don't draw inactive ghost
        if (!camera_) return; // Need camera for projection
        if (!textureLoaded_) return; // Texture missing
        ...
        float scale = static_cast<float>(pixelRect.w) / static_cast<float>(texRect.width); // Uniform scale to fit tile width
