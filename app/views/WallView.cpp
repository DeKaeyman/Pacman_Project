...
namespace pacman::app {
    ...
    void WallView::draw(sf::RenderWindow& window) {
        ...
        shape.setPosition(static_cast<float>(pixelRect.x), static_cast<float>(pixelRect.y)); // move to tile position
        shape.setSize(sf::Vector2f(static_cast<float>(pixelRect.w), static_cast<float>(pixelRect.h))); // Match tile size
