...
public:
    ...
    explicit ConcreteFactory(sf::RenderWindow& window) noexcept
        : window_(&window) {} // Stores reference to render window for view creation
    ...
    std::shared_ptr<logic::Ghost> createGhost(
        logic::GhostKind kind) override;                  // Creates Ghost model and attaches its view
