...
notImplementedModel() { // Placeholder for unimplemented model creation
  return nullptr;
}

std::shared_ptr<logic::PacMan> ConcreteFactory::createPacMan() {
  logic::Rect
      initial{}; // Default rectangle for Pacman until world positions it
...
ConcreteFactory::createGhost(logic::GhostKind kind) {
  logic::Rect initial{}; // Default rectangle for Ghost until world positions it
  auto m =
      std::make_shared<logic::Ghost>(initial, kind); // Create model instance
  attachViewToModel(m);                              // Connect model to view
...
std::shared_ptr<logic::Wall> ConcreteFactory::createWall() {
  auto m = std::make_shared<logic::Wall>(
      logic::Rect{});   // Create simple wall model with placeholder bounds
  attachViewToModel(m); // Connect model to view
  return m;             // Return Wall model
}

void ConcreteFactory::attachViewToModel(
    const std::shared_ptr<logic::PacMan> &pacman) {
  if (!pacman)
    return; // Nothing to attach if creation failed
  auto view =
      std::make_unique<PacManView>(pacman); // Create view tied to this model
...
void ConcreteFactory::attachViewToModel(
    const std::shared_ptr<logic::Ghost> &ghost) {
  if (!ghost)
    return; // Nothing to attach if creation failed
  auto view =
      std::make_unique<GhostView>(ghost); // Create view tied to this model
...
void ConcreteFactory::attachViewToModel(
    const std::shared_ptr<logic::Coin> &coin) {
  if (!coin)
    return; // Nothing to attach if creation failed
  auto view =
      std::make_unique<CoinView>(coin); // Create view tied to this model
...
void ConcreteFactory::attachViewToModel(
    const std::shared_ptr<logic::Fruit> &fruit) {
  if (!fruit)
    return; // Fruit to attach if creation failed
  auto view =
      std::make_unique<FruitView>(fruit); // Create view tied to this model
...
void ConcreteFactory::attachViewToModel(
    const std::shared_ptr<logic::Wall> &wall) {
  if (!wall)
    return; // Nothing to attach if creation failed

  auto view =
      std::make_unique<WallView>(wall); // Create view tied to this model
  views_.add(std::move(view)); // Register view so LevelState can draw it
}
} // namespace pacman::app