...
* [x] Detecteer **level clear**: alle coins & fruits opgegeten → verhoog level, bonuspunten, respawn items, ghosts naar center, behoud score & levens. Ghosts sneller, fear korter per level. 
...
* [x] Leg **input-mapping** vast in LevelState (app): pijlen zetten gewenste richting in World; geen logic in app; alleen doorgeven van intenties.
* [x] Plaats **Camera** gebruik in tekenpad: Views vragen Camera om wereld→pixel; pas window resize toe zonder logic aan te raken.
...
* [x] Voeg **scoreboard** persistentie toe: Score schrijft top-5 bij game-over; MenuState leest en toont boven “Play”. 
...
* [x] Implementeer **StartFlow**: App start in MenuState (scoreboard + Play); klik Play → LevelState init met Level 1, spawnt entiteiten via factory. 
* [x] Voeg **difficulty scaling** parameters toe in World: ghostSpeed += Δ, fearDuration *= factor<1 per level. Stel concrete waardes in en documenteer.
* [x] Bewaak **codekwaliteit**: smart pointers overal; expliciete initialisaties; const/override; geen memory leaks (virtuele destructors); referenties i.p.v. kopieën. Voeg clang-format config toe. 
* [x] Schrijf **exceptions** voor missende assets/config;
...
* [x] Documenteer **AI beslissingsboom** in comments en README (uitleg p en Manhattan regels).
* [x] Add delay before game start
* [x] Implementeer **pause overlay** met donker filter en menu (Resume/Restart/Menu).
